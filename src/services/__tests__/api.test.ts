...
describe('API Service', () => {
  ...
  describe('fetchUsers', () => {
    it('should fetch users with params', async () => {
      ...
      const result = await fetchUsers({ page: 2, limit: 10, search: 'foo' });
      expect(result).toEqual(mockResponse);
      expect(mockedAxios.get).toHaveBeenCalledWith('https://api.example.com/users', { params: { page: 2, limit: 10, search: 'foo' } });
    });
